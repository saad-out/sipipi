...
void typecast_str(std::string &str, std::string type) {
  ...
  if (type == "int") {
    if (container > std::numeric_limits<int>::max() ||
        container < std::numeric_limits<int>::min())
      std::cerr << "int overflow!!" << std::endl;
    else
      ScalarConverter::display(static_cast<int>(container));
  } else if (type == "float") {
    if (container > std::numeric_limits<float>::max() ||
        container < -std::numeric_limits<float>::max())
      std::cerr << "float overflow!!" << std::endl;
    else
      ScalarConverter::display(static_cast<float>(container));
  } else if (type == "double") {
    if (container > std::numeric_limits<double>::max() ||
        container < -std::numeric_limits<double>::max())
